...
def extract_text_from_xml(file_path):
    ...
    pbar = tqdm(total=total_pages, desc="Extraction des pages")
                ...
                if cleaned_text:
                    ...
                    try:
                        ...
                        for sentence in filtered_sentences:
                            sentence_id += 1
                            all_sentences.append([sentence_id, sentence, ''])
...
def write_to_csv(sentences, output_file):
    ...
    with open(output_file, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['id', 'phrase_fr', 'phrase_stereo'])
...
def post_process_csv(input_file, output_file):
    ...
    with open(input_file, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            pbar.update(1)
            if len(row) >= 2:  # S'assurer qu'il y a au moins id et phrase_fr
                phrase = row[1]
                ...
                if is_valid_sentence(phrase):
                    rows_to_keep.append([row[0], phrase, ''])
...
def validate_final_dataset(file_path, sample_size=100):
...
if __name__ == "__main__":
    ...
    output_file = 'data/output/wikipedia_phrases_initial.csv'
    ...
    final_output = 'data/output/wikipedia_phrases_clean.csv'

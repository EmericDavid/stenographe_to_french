    ...
    for word, punct in words_and_punct:
        if word:  # Si on a un mot
            if word not in word_to_steno:
                ...
                steno_word = word_to_steno[word]
                if punct and punct in word_to_steno:
                    
                    steno_word += " " + word_to_steno[punct]  # Ajouter la traduction de la ponctuation
                elif punct:
                    
                    steno_word += punct  # Ajouter la ponctuation brute si non traduite
                result.append(steno_word)
        elif punct:  # Si on a juste une ponctuation
            if punct in word_to_steno:
                
                result.append(word_to_steno[punct])
            else:
                result.append(punct)
...
def process_csv_file_with_multiprocessing(csv_filepath, word_to_steno):
    ...
    #modify the csv_path to wikipedia_output by wikipedia_translated
    csv_filepath_translated = csv_filepath.replace("wikipedia_output", "wikipedia_translated")

    # Écriture des phrases traduites dans un autre fichier
    with open(csv_filepath_translated, 'w', encoding='utf-8', newline='') as file:

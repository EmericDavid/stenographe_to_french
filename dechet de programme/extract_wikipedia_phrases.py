# data/extract_wikipedia_phrases.py
import os
import csv
import re
import logging
...
def is_valid_sentence(sentence):
    """
    Perform advanced validation on a sentence using multiple linguistic and structural criteria.
    
    Args:
        sentence (str): Sentence to validate
    
    Returns:
        bool: True if sentence passes all validation criteria, False otherwise
    """
    ...
    # Ignore biographical entries with multiple year ranges
    year_pattern = r'\(\d{4}\s*[-–]\s*\d{4}\)'
    year_matches = re.findall(year_pattern, sentence)
    if len(year_matches) > 1 and sentence.count(',') > 2:
        return False
...
def extract_text_from_xml(file_path):
    ...
    Returns:
        list: Extracted sentences with optional metadata
    """
    logging.info(f"Starting text extraction from {file_path}")
    ...
    pbar = tqdm(total=total_pages, desc="Extracting pages", unit="page")
    all_sentences = []
    sentence_id = 0
    ...
    try:
        for event, elem in context:
            if elem.tag.endswith('page'):
                # Trouver le titre et le texte (méthode corrigée)
                title = None
                text = None
...
def post_process_csv(input_file, output_file):
    """
    Post-process the CSV to remove problematic lines.
    ...
    with open(input_file, 'r', encoding='utf-8') as f:
        ...
        pbar = tqdm(total=total_rows, desc="Filtering sentences")
        
        for row in reader:
            pbar.update(1)
            phrase = row[1]
            
            # Remove quotes surrounding the entire phrase
            if phrase.startswith('"') and phrase.endswith('"'):
                phrase = phrase[1:-1]
            
            # Apply validation criteria again
    ...
    with open(output_file, 'w', newline='', encoding='utf-8') as f:
        ...
        for i, row in enumerate(rows_to_keep[1:], 1):
            writer.writerow(['', row[1]])
    
    total_kept = len(rows_to_keep) - 1
...
def process_wikipedia_dump(input_file, output_file):
    """
    ...
    try:
        ...
        # Extract sentences
        sentences = extract_text_from_xml(input_file)
